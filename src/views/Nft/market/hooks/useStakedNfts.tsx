...
export const useStakedNfts = (selectedPid: number) => {
  ...
  const nftStakeContractAddress = getRabbitCoinNftStakeAddress()
  ...
  const getNfts = async () => {
    try {
      ...
      const rawTokens = await multicallPolygonv1(smartNftStakeAddress ? smartNftStakeABI : rabbitCoinNftStakeABI, calls)

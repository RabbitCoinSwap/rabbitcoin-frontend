...
import { useRabbitCoinNFTContract } from 'hooks/useContract'
...
export const useNftsForCollectionAndAddress = (selectedPid: number) => {
  ...
  const collectionContract = useRabbitCoinNFTContract(collectionAddress)

  const getNfts = async () => {
    try {
      const tokenIds = nftPool['useApi']
        ? await walletOfOwnerApi(account, collectionAddress)
        : await collectionContract.walletOfOwner(account)
      ...
      if (nftPool['staticNftImage']) {
        ...
        tokenIdsNumber = await Promise.all(
  ...
  const { data, status, error, mutate } = useSWR(
    isAddress(account) ? [account, selectedPid, 'unstakedNftList'] : null,
    async () => getNfts(),
    {

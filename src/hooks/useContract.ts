...
import {
  ...
  getRabbitCoinNFTContract,
  getRabbitCoinPoolContract,
  getRabbitCoinAutoPoolVaultContract,
  getRabbitCoinContract,
  getRabbitCoinFarmContract,
  getRabbitCoinNftStakeContract,
  getRabbitCoinClaimRewardContract,
  getRabbitCoinClaimRewardV2Contract,
...
import { ChainId, WETH } from '@rabbitcoin/sdk'
import IRabbitCoinPairABI from '../config/abi/IRabbitCoinPair.json'
...
import { IRabbitCoinPair } from '../config/abi/types/IRabbitCoinPair'
...
export const useRabbitCoinNFTContract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinNFTContract(address, library.getSigner()), [library])
}
...
export const useRabbitCoin = (withSignerIfPossible = true) => {
  const { account, library } = useActiveWeb3React()
  return useMemo(
    () => getRabbitCoinContract(withSignerIfPossible ? getProviderOrSigner(library, account) : null),
    [account, library, withSignerIfPossible],
  )
}

export const useRabbitCoinClaimRewardContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinClaimRewardContract(library.getSigner()), [library])
}

export const useRabbitCoinClaimRewardV2Contract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinClaimRewardV2Contract(library.getSigner()), [library])
}
...
export const useRabbitCoinPool = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinPoolContract(library.getSigner()), [library])
}

// Only Farm Version Masterchef
export const useRabbitCoinFarm = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinFarmContract(library.getSigner()), [library])
}

// Nft Farm Version Masterchef
export const useRabbitCoinNftStake = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinNftStakeContract(library.getSigner()), [library])
}
...
export const useVaultPoolContract = (vaultKey: VaultKey): RabbitCoinAutoPoolVault | IfoPool => {
  const { library } = useActiveWeb3React()
  return useMemo(() => {
    return vaultKey === VaultKey.CakeVault
      ? getRabbitCoinAutoPoolVaultContract(library.getSigner())
...
// New
export const useRabbitCoinAutoPoolVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getRabbitCoinAutoPoolVaultContract(library.getSigner()), [library])
}
...
export function usePairContract(pairAddress?: string, withSignerIfPossible?: boolean): IRabbitCoinPair | null {
  return useContract(pairAddress, IRabbitCoinPairABI, withSignerIfPossible)
}

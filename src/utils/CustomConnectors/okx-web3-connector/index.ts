...
export class OkxConnector extends AbstractConnector {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    ...
    let account
    try {
      account = await (window.okxwallet.send as Send)('eth_requestAccounts').then(
  ...
  public async getChainId(): Promise<number | string> {
    ...
    let chainId
    try {
      chainId = await (window.okxwallet.send as Send)('eth_chainId').then(parseSendReturn)
    } catch {
    ...
    if (!chainId) {
      try {
        chainId = await (window.okxwallet.send as Send)('net_version').then(parseSendReturn)
      } catch {
    ...
    if (!chainId) {
      try {
        chainId = parseSendReturn((window.okxwallet.send as SendOld)({ method: 'net_version' }))
      } catch {
    ...
    if (!chainId) {
      if ((window.okxwallet as any).isDapper) {
        chainId = parseSendReturn((window.okxwallet as any).cachedResults.net_version)
      } else {
        chainId =
  ...
  public async getAccount(): Promise<null | string> {
    ...
    if (!account) {
      account = parseSendReturn((window.okxwallet.send as SendOld)({ method: 'eth_accounts' }))[0]
    }
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.okxwallet.send as Send)('eth_accounts').then((sendReturn) => {

...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://rabbitcoin.xyz - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/rabbitcoin.xyz)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://rabbitcoin.xyz'}
          color={text === 'https://rabbitcoin.xyz' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}

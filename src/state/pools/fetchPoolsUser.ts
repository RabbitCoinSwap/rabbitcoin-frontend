...
import multicall, { multicallPolygonv1 } from 'utils/multicall'
import { getRabbitCoinPoolContract, getMasterchefContract } from 'utils/contractHelpers'
...
export const fetchUserStakeBalances = async (account) => {
  ...
  const userInfo = await multicallPolygonv1(sousChefABI, calls)
  ...
  const { amount: masterPoolAmount } = await rabbitCoinPoolContract.userInfo(account)
...
export const fetchUserPendingRewards = async (account) => {
  ...
  // Cake / Cake pool
  const pendingReward = await rabbitCoinPoolContract.pendingReward(account)

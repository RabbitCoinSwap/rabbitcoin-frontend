...
import { getAddress, getRabbitCoinPoolAddress } from 'utils/addressHelpers'
import { getPoolApr } from 'utils/apr'
import { BIG_ZERO } from 'utils/bigNumber'
import { getRabbitCoinContract } from 'utils/contractHelpers'
...
const cakeContract = getRabbitCoinContract()
...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  ...
  const masterChefCalls = ['pendingReward', 'userInfo'].map((method) => ({
    address: getRabbitCoinPoolAddress(),
    name: method,
    params: ['0', account],
  }))
  const [[pendingReward], { amount: masterPoolAmount }] = await multicallPolygonv2(rabbitCoinPool, masterChefCalls)
    ...
    const liveData = poolsConfig.map((pool) => {
      ...
      const apr = !isPoolFinished
        ? getPoolApr(
            stakingTokenPrice,
            earningTokenPrice,
            Math.max(pool.participantThreshold ?? 0, getBalanceNumber(new BigNumber(totalStaking.totalStaked), pool.stakingToken.decimals)),

...
const getFarmsPrices = (farms: SerializedFarm[]) => {
  ...
  const rabbitUsdtFarm = farms.find((farm) => farm.pid === 1)
  const rabbitPriceUsdt = rabbitUsdtFarm.tokenPriceVsQuote ? BIG_ONE.div(rabbitUsdtFarm.tokenPriceVsQuote) : BIG_ZERO

  // CAUTION: This codes added by RabbitCoin to calculate matic/usdt price
  // If we have matic/usdt pair directly, we wont't need this anymore.
  const rabbitMaticFarm = farms.find((farm) => farm.pid === 2)
  const rabbitPriceMatic = rabbitMaticFarm.tokenPriceVsQuote ? BIG_ONE.div(rabbitMaticFarm.tokenPriceVsQuote) : BIG_ZERO
  const maticPriceUsdt = rabbitPriceMatic.div(rabbitPriceUsdt)

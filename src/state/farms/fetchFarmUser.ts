...
import rabbitCoinFarmABI from 'config/abi/rabbitCoinFarm.json'
import { multicallPolygonv1 } from 'utils/multicall'
import { getAddress, getRabbitCoinFarmAddress } from 'utils/addressHelpers'
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawEarnings = await multicallPolygonv1(rabbitCoinFarmABI, calls)
